<?php
    ...
    // 1. Proteksi Halaman: Hanya user yang sudah login (Level 1 atau Level 2) yang boleh mengakses.
    if (!isset($_SESSION['login'])){
        header('Location: login.php');
        exit;
    } 
    // Jika hanya Level 1 yang boleh lihat semua laporan:
    // if ($_SESSION['role'] != 1) { 
    ...
    function ambil_laporan($koneksi, $mulai, $selesai) {
        
        // 1) Total per tanggal
        // NOTE: Asumsi nama kolom di tabel 'transaksi' adalah 'waktu_transaksi' dan 'total'
        $query_list = "SELECT t.waktu_transaksi AS tanggal, SUM(t.total) AS total_per_tanggal
                FROM transaksi t
        ...
        mysqli_stmt_bind_param($stmt_list, "ss", $mulai, $selesai);
        ...
        $query_total = "SELECT IFNULL(SUM(total),0) AS total_all FROM transaksi WHERE waktu_transaksi BETWEEN ? AND ?";
        $stmt_total = mysqli_prepare($koneksi, $query_total);
        mysqli_stmt_bind_param($stmt_total, "ss", $mulai, $selesai);
        mysqli_stmt_execute($stmt_total);
        $result_total = mysqli_stmt_get_result($stmt_total);
        $row_total = mysqli_fetch_assoc($result_total);
        $total_kumulatif = $row_total ? $row_total['total_all'] : 0;

        // 3) Jumlah pelanggan unik dalam rentang
        $query_pelanggan = "SELECT COUNT(DISTINCT pelanggan_id) AS jumlah_pelanggan FROM transaksi WHERE waktu_transaksi BETWEEN ? AND ?";
        $stmt_pelanggan = mysqli_prepare($koneksi, $query_pelanggan);
        mysqli_stmt_bind_param($stmt_pelanggan, "ss", $mulai, $selesai);
        ...
        $jumlah_pelanggan = $row_pelanggan ? $row_pelanggan['jumlah_pelanggan'] : 0;

        return [$data_list, $total_kumulatif, $jumlah_pelanggan];
    }
