...
$stmt_barang = mysqli_prepare($conn, 
    "SELECT nama_barang, harga, stok, supplier_id 
...
if (mysqli_num_rows($result_barang) > 0) {
    $data = mysqli_fetch_assoc($result_barang);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt_update = mysqli_prepare($conn, 
        "UPDATE barang 
         SET nama_barang = ?, harga = ?, stok = ?, supplier_id = ?
         WHERE id = ?"
    );

    mysqli_stmt_bind_param($stmt_update, "siiii",
        $nama_barang, $harga, $stok, $supplier_id, $id
    );
...
<form method="POST">
    ...
    <button type="submit" class="btn-save">Update Barang</button>
