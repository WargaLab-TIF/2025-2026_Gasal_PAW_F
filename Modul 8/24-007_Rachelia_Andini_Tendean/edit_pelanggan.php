...
$data_lama_q = mysqli_query($koneksi, "SELECT * FROM pelanggan WHERE id='$id'");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"])) {
    ...
    if (empty($errors)) {
        ...
        if (mysqli_query($koneksi, $query)) {
            ...
            // Ambil ulang data terbaru untuk form
            $data_lama = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT * FROM pelanggan WHERE id='$id'"));
            $nama = $data_lama['nama'];
            $telp = $data_lama['telp'];
            $alamat = $data_lama['alamat'];
            $jenis_kelamin = $data_lama['jenis_kelamin'];
...
<body>
    ...
    <div style="padding: 20px;">
        ...
        <?php if (!empty($errors)): ?><div class="error"><ul><?php foreach ($errors as $e): ?><li><?= htmlspecialchars($e) ?></li><?php endforeach; ?></ul></div>
        <?php elseif ($pesan): ?><div class="pesan"><?= htmlspecialchars($pesan) ?></div>
        <?php endif; ?>
