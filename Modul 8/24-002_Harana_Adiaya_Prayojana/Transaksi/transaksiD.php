<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        if(!$err){
            ...
            $stok=mysqli_query($koneksi,"SELECT stok FROM barang WHERE id=$barang");
            $harSat=(int)mysqli_fetch_assoc($harB)['harga'];
            $setStok=(int)mysqli_fetch_assoc($stok)['stok'];
            $minStok=$setStok-$qty;
            echo $minStok;
            $hasil=$harSat*$qty;
            ...
            if($simpan){
                ...
                mysqli_query($koneksi,"UPDATE barang SET stok = $minStok WHERE id=$barang");
