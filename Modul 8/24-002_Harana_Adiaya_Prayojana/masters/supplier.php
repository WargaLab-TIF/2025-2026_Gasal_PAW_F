...
<main class="layout">
    ...
    <br>
    <div style="display:flex; align-items:center; margin-bottom:10px;">
        <form action="./add/addsup.php" ><button class="adding">Tambah User</button></form>
    </div>
    <table border="1" cellpadding="8">
        <tr>
            <th>No</th><th>id</th><th>Nama</th><th>telp</th><th>alamat</th><th>Tindakan</th>
        </tr>
        <?php
            $no=1;
        ...
        <tr>
            ...
            <td>
                <div style="display:flex; align-items:center; margin-bottom:10px; justify-content: space-between;">
                    <button class="detail" onclick="location.href='./edit/edsup.php?id=<?=$row['id']?>'">Edit</button>
                    <button class="del" onclick="if(confirm('Yakin ingin hapus Supplier <?=$row['nama']?>')) location.href='?delete=<?=$row['id']?>'">Hapus</button>
                </div>
            </td>
        </tr>
        <?php endwhile; ?>
