...
<main class="layout">
    ...
    <br>
    <div style="display:flex; align-items:center; margin-bottom:10px;">
        <form action="./add/addbar.php" ><button class="adding">Tambah Barang</button></form>
    </div>
    <table border="1" cellpadding="8">
        <tr>
            <th>No</th><th>ID</th><th>Nama Barang</th><th>Harga</th><th>stok</th><th>Nama Supplier</th><th>Tindakan</th>
        </tr>
        <?php
        ...
        <tr>
            ...
            <td>
                <div style="display:flex; align-items:center; margin-bottom:10px; justify-content: space-between;">
                    <button class="detail" onclick="location.href='./edit/edbar.php?id=<?=$row['id']?>'">Edit</button>
                    <button class="del" onclick="if(confirm('Yakin ingin hapus Barang <?=$row['nama_barang']?>?')) location.href='?delete=<?=$row['id']?>'">Hapus</button>
                </div>
            </td>
        </tr>
        <?php endwhile; ?>
