...
// Cek sudah login?
if (!isset($_SESSION['login']) || $_SESSION['login'] != true || $_SESSION['level'] != 2) {
...
$nama_user = $_SESSION['username'];
    ...
    // Cek apakah tombol "Tampilkan" sudah diklik
    if (isset($_GET['filter']) && !empty($_GET['tgl_mulai']) && !empty($_GET['tgl_selesai'])) {
        $show_report = true;
        $tgl_mulai = $_GET['tgl_mulai'];
        $tgl_selesai = $_GET['tgl_selesai'];

        // Query 1: Data Harian untuk Grafik & Tabel (GROUP BY tanggal)
        // Sesuai gambar: Menampilkan total per hari
        ...
        // Query 2: Total Kumulatif (Jumlah Pelanggan Unik & Total Uang)
